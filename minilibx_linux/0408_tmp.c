...
int		write_bmp_header(int fd, int file_size, t_vars *vars)
{
	...
	bmpfileheader[13] = (unsigned char)(1);
	bmpfileheader[14] = (unsigned char)(24);
...
int		get_color(t_vars *vars, int x, int y)
{
	...
	color = *(int*)(vars->img.img_ptr
			+ (4 * vars->window_width * (vars->window_height - 1 - y))
			+ (4 * x));
...
int	save_bmp(t_vars *vars)
{
	...
	if (!(fd != open("save.bmp", O_WRONLY | O_CREAT, 0744)))
		return (0);
	file_size = 54 + (3 * (vars->window_width + pad) * vars->window_height);
	if (!write_bmp_header(fd, filesize, vars))
		return (0);
