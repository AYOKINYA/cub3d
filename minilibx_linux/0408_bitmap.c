...
/*   Updated: 2020/04/10 16:33:05 by jkang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprites(t_sprite_calc *calc, t_vars *vars)
{
	...
	while (stripe < calc->draw_x_end)
      {
        calc->stripe_tex_x = (int)(256 * (stripe - (calc->sprite_width / -2 + calc->spritescreenX)) * vars->tex[4].img_width / calc->sprite_width) / 256;
...
void	draw_tex_on_walls(t_vars *vars, int x, int index)
{
	...
	while (y < vars->drawEnd)
	{
		...
		if ((int)vars->texY >= 0 && (int)vars->texY <= tex_height - 1 && y >= 0 && y <= vars->window_height)
		{
...
int	free_all_loads(t_vars *vars)
{
    ...
    // while (i < 5)
	// {
	// 	if (vars->tex[i].data != 0)
	// 		mlx_destroy_image(vars->mlx, vars->tex[i].img_ptr);
	// 		//ft_bzero(vars->tex[i].data, sizeof(int) * vars->tex[i].img_width * vars->tex[i].img_width);
	// 	++i;
	// }
	free(vars->tex);
	free(vars->sprites);
...
int	load_img(t_vars *vars, t_config *reading)
{
	...
	if (!(vars->tex[0].img_ptr = mlx_xpm_file_to_image(vars->mlx, reading->north_path, &vars->tex[0].img_width, &vars->tex[0].img_height)))
		return (0);
 	vars->tex[0].data = (int *)mlx_get_data_addr(vars->tex[0].img_ptr, &vars->tex[0].bpp, &vars->tex[0].size_line, &vars->tex[0].endian);
	if (!(vars->tex[1].img_ptr = mlx_xpm_file_to_image(vars->mlx, reading->south_path, &vars->tex[1].img_width, &vars->tex[1].img_height)))
		return (0);
 	vars->tex[1].data = (int *)mlx_get_data_addr(vars->tex[1].img_ptr, &vars->tex[1].bpp, &vars->tex[1].size_line, &vars->tex[1].endian);
	if (!(vars->tex[2].img_ptr = mlx_xpm_file_to_image(vars->mlx, reading->east_path, &vars->tex[2].img_width, &vars->tex[2].img_height)))
		return (0);
 	vars->tex[2].data = (int *)mlx_get_data_addr(vars->tex[2].img_ptr, &vars->tex[2].bpp, &vars->tex[2].size_line, &vars->tex[2].endian);
	if (!(vars->tex[3].img_ptr = mlx_xpm_file_to_image(vars->mlx, reading->west_path, &vars->tex[3].img_width, &vars->tex[3].img_height)))
		return (0);
 	vars->tex[3].data = (int *)mlx_get_data_addr(vars->tex[3].img_ptr, &vars->tex[3].bpp, &vars->tex[3].size_line, &vars->tex[3].endian);
	if (!(vars->tex[4].img_ptr = mlx_xpm_file_to_image(vars->mlx, reading->sprite_path, &vars->tex[4].img_width, &vars->tex[4].img_height)))
		return (0);
...
int		extract_color(t_vars *vars, int x, int y)
{
	...
	color = vars->img.data[x + (vars->window_height - 1 - y) * vars->window_width];
...
int	play_on_screen(t_vars *vars, t_config *reading, int save_flag)
{
	vars->mlx = mlx_init();
...
int main(int argc, char *argv[])
{
	...
	if (!config_to_vars(vars, reading))
		return (free_all(vars, reading));
	if (!play_on_screen(vars, reading, save_flag))
	{
		free_all_loads(vars);
		return (free_all(vars, reading));
	}
