...
int final_check_elements(t_config *reading)
{
	...
	if (check != 1)
		return (0);
	check = check * reading->map_start_flag * reading->map_end_flag * reading->surrounding_flag; // * position_flag;
	if (check != 1)
		return (0);
	return (check);
}
...
int		**creat_map_to_validate(t_config *reading, t_list *map, t_vars *vars, int col_max)
{
	int i;
	int j;
	int row_max;
	...
	if (!(test = (int **)malloc(sizeof(int *) * row_max)))
		return (0);
	malloc_test_map(test, row_max, col_max);
...
int check_surrounding(t_config *reading, t_list *map, t_vars *vars)
{
	t_list	*temp;
	int		col_max;
	int		row_max;
	...
	if(!(test = creat_map_to_validate(reading, map, vars, col_max)))
		return (0);
...
int	copy_map(t_config *reading, t_list *map, t_vars *vars)
{
	int i;
	int j;
	int col_max;
	...
	while (i < reading->map_row_size)
	{
		if (!(vars->map[i] = (int *)malloc(sizeof(int) * map->col_size)))
		{
			j = 0;
			while (j < i)
			{
...
int read_config_body(t_list *map, t_vars *vars, t_config *reading, char *argv)
{
	if (!read_map(map, reading, argv))
		return (0);
	reading->map_row_size = ft_lstsize(map);
	if (!copy_map(reading, map, vars))
		return (0);
	if (!check_surrounding(reading, map, vars))
		return (0);
...
int	read_config(t_vars *vars, t_config *reading, char *argv)
{
	...
	map->col_size = 0;
	map->next = 0;
	if (!read_config_body(map, vars, reading, argv))
	{
